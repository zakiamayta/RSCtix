...
@section('content')
<div id="page-alert" class="alert alert-danger fade-in text-center rounded-3">
    Nomor HP harus 9–11 digit.
</div>
                    ...
                    <div class="carousel-inner">
                      @forelse($varian->images ?? [] as $idx => $img)
                        <div class="carousel-item @if($idx === 0) active @endif">
                          <img src="{{ asset('images/events/' . $img->url) }}" class="d-block w-100" style="height:200px; object-fit:cover;">
                        </div>
                      @empty
        ...
        <div class="card shadow-sm border-0 rounded-3 mb-4">
          ...
          <div class="card-body">
            <form id="checkoutForm" method="POST" action="{{ route('merch.preview') }}">
              ...
              <div class="text-end">
                {{-- ✅ Tombol Checkout awalnya disabled --}}
                <button type="submit"
                        id="checkoutBtn"
  ...
  orderItems.forEach((item, index) => {
    ...
    html += `
      <div class="order-item p-3 mb-3 fade-in position-relative card">
        <button type="button" class="btn btn-sm position-absolute top-0 end-0 m-2 rounded-circle trash-btn" 
        ...
        <p class="mb-2">Harga Satuan: Rp ${item.harga.toLocaleString()}</p>
        ...
        <p class="fw-semibold text-end mb-0 text-orange">Subtotal: Rp ${subtotal.toLocaleString()}</p>
...
document.querySelectorAll('.btn-add').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    currentVarian.ukurans.forEach(u=>{
      if(u.stok>0){
        html+=<button type="button" class="btn w-100 mb-2 btn-orange-pill" onclick="selectUkuran(${u.id}, '${u.ukuran}', ${u.harga})" data-bs-dismiss="modal">${u.ukuran} - Rp ${u.harga.toLocaleString()}</button>;
      } else {
